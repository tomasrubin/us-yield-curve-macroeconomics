function specCrossDensity = fEstimateSpecCrossDensity_holdout_NaNready(censor,onb,response,response_mean,mu_est_inSpace,bw_F,nGridFreq,fit_on_proportion)

% where to start
fit_start = ceil( censor.nGridTime * fit_on_proportion(1) + 0.001);

% holdout = percentage of how much data at the end of the response process is not used for the training
holdout = 1-fit_on_proportion(2); % first element = the end of the first partition
...
disp("crossDensity: fitting on "+num2str(fit_start)+"..."+num2str(holdout_start-1))
...
specCrossDensity.q_bartlett = floor( 1* censor.nGridTime^(1/3));
...
for lag_real = (-numOfLags+1):(numOfLags-1)
    ...
    lower_bound = max(fit_start,fit_start-lag_real);
    upper_bound = min(censor.nGridTime-lag_real,holdout_start-1); % holdout bound
    
    for t = lower_bound:upper_bound
        if ~isnan(response(t+lag_real))
            nRawCovs(lag_index) = nRawCovs(lag_index) + sum( censor.nGrid(t) );
        end
    end
...
for lag_real=(-specCrossDensity.q_bartlett+1):(specCrossDensity.q_bartlett-1)
    ...
    for tau_jj = 1:onb.nGridSmoother
        ...
        % save Q
        for r = 0:1
            save_Q(lag_index,r+1,tau_jj) = 1/( censor.nGridTime - abs(lag_real) ) * ...
                sum(...
                Y{lag_index} .* ...
                kern .* ...
                ((dev)/bw_F).^r);
        ...
        % save S
        for r = 0:2
            save_S(lag_index,r+1,tau_jj) = 1/( censor.nGridTime - abs(lag_real) ) * ...
                sum(...
                kern .* ...
                ((dev)/bw_F).^r);
        end
