function [bw_c_opt, crossCov0_smoother_opt] = fKCVforCrossR_NaNready(censor,onb,response,mu_est_inSpace)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% pre-calculate some quantities

% data counts for training and validation and all
nData_train = zeros(censor.cv_K,1);
nData_valid = zeros(censor.cv_K,1);
nData_all = 0;

for kk = 1:censor.cv_K
    
    for tt = 1:censor.nGridTime
        
        % only count if the response in "t" is available
        if ~isnan(response(tt))
            
            % cycle trhough all observation on "X" in time "t"
            for iii = 1:censor.nGrid(tt)
                                
                if censor.cv_batch{tt}(iii) ~= kk % if it is NOT in batch "kk" => it's train
                    nData_train(kk) = nData_train(kk) + 1;
                else % is in batch "kk" => validation
                    nData_valid(kk) = nData_valid(kk) + 1;
                end                                
            end
        end        
    end    
end

% count "nData_all"
for kk = 1:censor.cv_K
    for tt = 1:censor.nGridTime
        % only count if the response in "t" is available
        if ~isnan(response(tt))
            nData_all = nData_all + 1;
        end
    end
end


% prepare the data pool for K-folds
Xs_train = cell(censor.cv_K,1); % here will be the values on [0,1] - for training - all except partition k
Xs_valid = cell(censor.cv_K,1); % for validation - partition k only
Xg_train = cell(censor.cv_K,1); % intex of the grid
Xg_valid = cell(censor.cv_K,1);
Y_train = cell(censor.cv_K,1); % value of the function
Y_valid = cell(censor.cv_K,1);

% save all data - it will be used for the final fit on all data
Xs_all = nan( nData_all,1 );
Xg_all = nan( nData_all,1 );
Y_all = nan( nData_all,1 );


% cycle for each partition
for kk = 1:censor.cv_K    
    % prepare data structures
    Xs_train{kk} = nan( nData_train(kk) , 1);
    Xg_train{kk} = nan( nData_train(kk) , 1);
    Y_train{kk} = nan( nData_train(kk) , 1);
    Xs_valid{kk} = nan( nData_valid(kk) , 1);
    Xg_valid{kk} = nan( nData_valid(kk) , 1);
    Y_valid{kk} = nan( nData_valid(kk) , 1);
    
    % counters of current data points
    iData_train = 1;
    iData_valid = 1;
    iData_all = 1;
    
    % fill-in the data
    for tt = 1:censor.nGridTime
        
        % only count if the response in "t" is available
        if ~isnan(response(tt))
            
            for iii = 1:censor.nGrid(tt)
                if censor.cv_batch{tt}(iii) ~= kk
                    % TRAINING SET : include iff it's not "kk"
                    Xs_train{kk}(iData_train) = onb.gridSpace(censor.grid{tt}(iii));
                    Xg_train{kk}(iData_train) = censor.grid{tt}(iii);
                    Y_train{kk}(iData_train) = (censor.data{tt}(iii) - mu_est_inSpace(censor.grid{tt}(iii))) * response(tt);
                    iData_train = iData_train + 1;
                else
                    % VALIDATION SET : include iff it's "kk"
                    Xs_valid{kk}(iData_valid) = onb.gridSpace(censor.grid{tt}(iii));
                    Xg_valid{kk}(iData_valid) = censor.grid{tt}(iii);
                    Y_valid{kk}(iData_valid) = (censor.data{tt}(iii) - mu_est_inSpace(censor.grid{tt}(iii))) * response(tt);
                    iData_valid = iData_valid + 1;
                end
                % ALL CASES
                Xs_all(iData_all) = onb.gridSpace(censor.grid{tt}(iii));
                Xg_all(iData_all) = censor.grid{tt}(iii);
                Y_all(iData_all) = (censor.data{tt}(iii) - mu_est_inSpace(censor.grid{tt}(iii))) * response(tt);
                iData_all = iData_all + 1;
            end
        end
    end
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Baysian optimization

% perform the optimization
f = @(x)sum_of_squares(x.bw_c); % I don't know why but I have to create the anonymous function handle
bw_c = optimizableVariable('bw_c',[0.02,.9]);
results = bayesopt(f, bw_c, 'PlotFcn', [], 'MaxObjectiveEvaluations', 10, 'IsObjectiveDeterministic', true);
% results = bayesopt(f, bw_c, 'MaxObjectiveEvaluations', 10, 'IsObjectiveDeterministic', true);

% return the optimal BW and its estimated mu
bw_c_opt = results.XAtMinObjective.bw_c;


%% fit again on full data
crossCov0_smoother_opt = fSmootherLine( Xs_all, Y_all, bw_c_opt, 1,onb.gridSmoother  );

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%   function to calculate the CV score for a given candidate bandwidth
    function mse = sum_of_squares(bw_c)

        mse = 0; % the mean square error
        for k = 1:censor.cv_K
            
            % perform the line smoother - on the training data partition except k
            crossCov0_est = fSmootherLine(Xs_train{k},Y_train{k},bw_c,1,onb.gridSmoother);
            crossCov0_est_inSpace = onb.onbMatrix*onb.smoothingMatrix_gridSmoother2ONB* crossCov0_est;
            
            % calculate the sum of squares - on validation partition k
            mse = mse + 1/censor.cv_K * mean(( crossCov0_est_inSpace(Xg_valid{k}) - Y_valid{k} ).^2);
        end
        
    end

end