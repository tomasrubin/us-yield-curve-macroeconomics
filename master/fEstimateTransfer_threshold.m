function transfer = fEstimateTransfer_threshold(onb,specDensity,specCrossDensity, threshold_multiplier)
...
for k=1:transfer.nGridFreq
    ...
    % invert only those eigenvalues that are above this threshold
    threshold = 1/specDensity.q_bartlett * threshold_multiplier;
    
    % spectral truncation of M
    M = (squeeze(specDensity.ONB(k,:,:)) + squeeze(specDensity.ONB(k,:,:))')/2;
    [V,D] = eig(M);
    diagD = diag(D);
    truncationEigen = sum(diagD > threshold); % count how many eigenvalues are above the threshold
