function [kriging, kriging_padded] = fKrigingX(censor,onb,covLags,mu_est_ONB,sigma_est,dynamic,calculate_var, dataFull_padding)
...
if dynamic %%          dynamic kriging
    ...
    % vectorize the functional time series = number of basis coefficients
    dimX = onb.nBasis * nGridTime_padded;
    
    % calculate the number of nonzero elements
    nonzeros = 0;
    for lag = 0:covLags.cutOfLag
        nonzeros = nonzeros + (nGridTime_padded-lag)*onb.nBasis^2;
    end
    
    % prepare the vectors for sparse matrix
    pos_i = zeros(1,nonzeros);
    pos_j = zeros(1,nonzeros);
    v = zeros(1,nonzeros);
    current = 1;
    
    % go over the nonzero elements
    disp(['fKrigingX: preparing prior_covX, cutOfLag = ',num2str(covLags.cutOfLag)])
    for lag = 0:covLags.cutOfLag
...
%                     prior_covX( (t-1)*onb.nBasis + ii, (s-1)*onb.nBasis +jj) = covAct(jj,ii); %covAct(ii,jj);                    
                    pos_i( current ) = (t-1)*onb.nBasis + ii;
                    pos_j( current ) = (s-1)*onb.nBasis +jj;
                    v( current ) = covAct(jj,ii);
                    current = current + 1;

%                     prior_covX( (s-1)*onb.nBasis + ii, (t-1)*onb.nBasis +jj) = covAct(ii,jj); %covAct(jj,ii);
                    pos_i( current ) = (s-1)*onb.nBasis + ii;
                    pos_j( current ) = (t-1)*onb.nBasis + jj;
                    v( current ) = covAct(ii,jj);
                    current = current + 1;
                end
            end
        end
    ...
    prior_covX = sparse(pos_i,pos_j,v,dimX,dimX);
    clear pos_i pos_j v;
    ...
    % calculate the number of nonzero elements
    nonzeros = sum(censor.nGrid) * onb.nBasis;
    pos_i = zeros(1,nonzeros);
    pos_j = zeros(1,nonzeros);
    v = zeros(1,nonzeros);
    current = 1;
...
%                 obsOperator( start_skip_Z(t)+jj , dataFull_padding*onb.nBasis + (t-1)*onb.nBasis + ii ) = H_act(jj,ii);
                pos_i( current ) = start_skip_Z(t)+jj;
                pos_j( current ) =  dataFull_padding*onb.nBasis + (t-1)*onb.nBasis + ii;
                v( current ) = H_act(jj,ii);
                current = current + 1;
            end        
        end
    end

    obsOperator = sparse(pos_i, pos_j, v, dimZ, dimX);
    clear pos_i pos_j v;
